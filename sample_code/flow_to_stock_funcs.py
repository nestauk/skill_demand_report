...
a stock. For a more detailed explanation of the models, see Nesta's report on
...
def get_stock_breakdown(data, agg_func = 'sum', agg_col = 'vacancy_weight',
    ...
    and then discard the first two months of stock.
    ...
    adjustment weights and NOT the final stock results. However, it might make it
...
def scale_weights_by_total_levels(weights_df, ons_vacancies, raw_stock_per_month_sic,
    ...
    we checked and the median is very close to the mean computed after removing
